...
namespace NetZlib
{
    ...
    sealed class Deflate
    {
        ...
        readonly Tree l_desc = new Tree();  // desc for literal tree
        readonly Tree d_desc = new Tree();  // desc for distance tree
        readonly Tree bl_desc = new Tree(); // desc for bit length tree
        ...
        GZIPHeader GetGZIPHeader()
        {
            lock (this)
            {
                return this.gheader ?? (this.gheader = new GZIPHeader());
            }
        }
    }
}
